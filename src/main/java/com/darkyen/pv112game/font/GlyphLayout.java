...
public final class GlyphLayout {
	...
	private GlyphRun pushRun (float targetWidth, GlyphRun previous, Color color, boolean forceNewLine) {
		...
		newRun.color = color.toFloatBits();
		return newRun;
	}

	public void setText (CharSequence str, Color color, float targetWidth, int hAlign) {
	...
	public void setText (CharSequence str, int start, int end, Color color, float targetWidth, int hAlign) {
		{// Setup & cleanup
			width = 0f;
			height = 0f;
	...
	public void draw (SpriteBatch batch, float x, float y) {
		for (GlyphRun run : runs) {
			...
			for (int g = 0; g < glyphCount; g++) {
				...
				batch.useTexture(font.pages[glyph.pageIndex]);

				batch.draw(
						runX + glyphXOffset,
						runYBaseline - glyph.yOffset,
						glyph.pageWidth, -glyph.pageHeight,
	...
	private static final class GlyphRun implements Pool.Poolable {
		...
		/** True if this starts with \n. One run can contain only one linebreak, {@link #pushRun(float, GlyphRun, Color, boolean)}
		 * takes care of that.  */
		boolean hasLinebreak () {
			return codepoints.size > 0 && codepoints.first() == '\n';
		}

		/** True if this starts with \t. One run can contain only one tab, {@link #pushRun(float, GlyphRun, Color, boolean)} takes
		 * care of that.  */
		boolean hasTab () {
			return codepoints.size > 0 && codepoints.first() == '\t';
		}
